...
"""适用场景：
运动分析：分析运动员的动作、姿势，评估运动的效率与健康状况。

物理学研究：研究人体的运动学，分析肌肉的负载与运动表现。

运动指导：根据分析结果给出如何改进动作的建议，比如运动训练、康复等。"""
class Analyzer():
    ...
    def __init__(self, mode=False, upBody=False, smooth=True, detectionCon=0.5, trackCon=0.5):
        ...
        self.pose = self.mpPose.Pose(
            static_image_mode=self.mode,  # Whether to process static images (False by default)
            
            smooth_landmarks=self.smooth,  # Whether to smooth landmarks (True by default)
            min_detection_confidence=self.detectionCon,  # Minimum detection confidence (0.5 by default)
            min_tracking_confidence=self.trackCon  # Minimum tracking confidence (0.5 by default)
        )
    ...
    def find_all_positions(self, cap):
        positions = []  # 改为一个列表，用于收集所有帧的数据
        while True:
            success, img = cap.read()

            if not success:
                break  # 如果没有读取到图片，退出循环

            img = self.findPose(img)  # 提取姿势
            raw_position = self.findPosition(img, draw=False)  # 获取每个关节的位置

            # 清理并转换位置
            clean_position = [(pos[1], pos[2]) for pos in raw_position]  # 提取 (x, y) 坐标

            # 创建一个字典来表示每个位置
            row = {i: clean_position[i] for i in range(33)}

            # 将每一行添加到 positions 列表中
            positions.append(row)

        # 将所有行合并成一个 DataFrame
        positions_df = pd.DataFrame(positions)

        self.all_positions = positions_df  # 将最终的 DataFrame 存储到 self.all_positions
    ...
    def output_video(self, name='output', limbs=[LEG_LOWER_RIGHT, LEG_UPPER_RIGHT, UPPER_BODY_RIGHT], out_frame_rate=12):
        ...
        # 修改输出路径为 .mp4 格式
        outpath = 'result_musle_activate/' + name + '.mp4'
        
        # 使用 X264 编码器（如果没有支持 X264，可以改为 'mp4v'）
        fourcc = cv2.VideoWriter_fourcc(*'X264')  # 或者使用 'mp4v' 替代 'X264'，具体取决于你的环境支持

        # 设置视频写入对象，指定输出路径、编码器、帧率和尺寸
        out = cv2.VideoWriter(outpath, fourcc, out_frame_rate, (self.width, self.height))
        ...
        while True:
            ...
            for i, joint in enumerate(self.joints_analyzed):
                ...
                img = cv2.putText(img, joint_to_text[joint], org=(int(0.1 * self.width) + shift_horizontal_text, int(0.9 * self.height)),
                                fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(0, 0, 255), thickness=2, lineType=cv2.LINE_AA)
            ...
            img = cv2.putText(img, 'Overall(wrist)', org=(int(0.1 * self.width), int(0.9 * self.height) + 50),
                            fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=1, color=(0, 0, 255), thickness=2, lineType=cv2.LINE_AA)  # Text
    ...
    def output_graph(self,name = 'mechanics analysis', cols_to_analyze = "All", scaling = {'size': True, 'strength': False, 'framerate': True }):
        ...
        fig.savefig(('result_musle_activate/' + name + '.png'))
